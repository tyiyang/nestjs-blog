...
export class ArticleService {
  ...
  async findFeed(userId: number, query): Promise<ArticlesRO> {
    const _follows = await this.followsRepository.findBy({
      followerId: userId,
    });
  ...
  async favorite(id: number, slug: string): Promise<ArticleRO> {
    let article = await this.articleRepository.findOneBy({ slug });
    // 当你在查询 UserEntity 时，要确保将 relations 设置为 ['favorites'] 以加载 favorites 关联数据
  ...
  async update(slug: string, articleData: any): Promise<ArticleRO> {
    const toUpdate = await this.articleRepository.findOneBy({ slug });
